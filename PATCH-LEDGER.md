...
- **Slide 05's area fill is not zero-baselined** — the fill bottom sits at roughly $59B, so
  the shaded area overstates magnitude even though every point is labelled. The dashed
  reference now marks the 2026 level explicitly, so a reader can see the comparison starts
  at $83.7B, but the fill still runs to an unlabelled floor. Fine for a line, arguable for
  an area. A decision, not a defect.
